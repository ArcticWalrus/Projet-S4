...
int Accel_raw_to_g_float(int nbDonnees)
{
	...
	compteur++;
	if (compteur % 4 == 0) {
		float moye = accelYYY/4;
		//printf("Valeur sommee %3.2f       | Valeur moyenne %3.2f\n", accelYYY, moye);
		if (moye < -0.15) {
			//printf("\n\n\nDROIIIIITEEEEEEEEEEE\n\n\n");
		}
		else if (moye > 0.15) {
					//printf("\n\n\nGAUCHHHHEEEEEEE\n\n\n");
				}
		sommeTotal = sommeTotal + accelYYY;
		if (compteur == nbDonnees*4) {
			moyeTotal = sommeTotal/compteur;
			//printf("Valeur sommee Total: %3.2f       | Valeur moyenne Total:  %3.2f\n", sommeTotal, moyeTotal);
		} else {
			accelYYY = 0;
			moye = 0;
		}
