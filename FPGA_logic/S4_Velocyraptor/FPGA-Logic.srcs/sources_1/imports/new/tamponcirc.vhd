...
entity tamponcirc is
    Port (
            i_clk, it_reset     : in std_ulogic; --i_decrement_stb supposée être à 200Hz. durée de vie des 
            i_decrement_stb     : in std_ulogic;                                                 --échantillons seraient de 2 secondes (400 iter)
...
architecture comport of tamponcirc is
    type RAM is array (natural range <>) of unsigned(8 downto 0);
    constant max: natural := 400;
    constant taille_tampon: natural := 63;
	signal tampon: RAM(0 to 63) := (others =>  to_unsigned(max,9));
    ...
    signal donnee_tete: unsigned (8 downto 0); -- donnee à la position "tete"
begin
    process( i_clk, it_reset )
    begin
        if it_reset='1' then
            tampon <= (others => to_unsigned(max,9));
            items <= (others => '0');
            tete <= (others => '0');
            queue <= (others => '0');
            o_plein <= '0';
            ...
            if i_WR = '1' then
                if items < to_unsigned(taille_tampon, 6) then
                    tampon( to_integer( queue )) <= to_unsigned(max,9);
                    queue <= queue + "000001";
                    items <= items + "000001";
                    o_plein <= '0';
                    o_vide <= '0';
            ...
            if i_decrement_stb='1' then
                for I in 0 to actif'length-1 loop
                    if actif(I) = '1'and tampon(I) > "000000000" then
                        tampon(I) <= tampon(I) - "000000001";
                    end if;
                end loop;
            end if;
            ...
            if donnee_tete = "000000000" then
                tampon(to_integer(tete)) <= to_unsigned(max,9);
                tete <= tete + "000001";
                items <= items - "000001";
                o_vide <= '0';
                o_plein <= '0';
